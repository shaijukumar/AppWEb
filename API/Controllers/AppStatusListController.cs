...
namespace API.Controllers
{
    ...
    {       
        ...
        [HttpPost]        
		public async Task<ActionResult<AppStatusListDto>> Create(Create.Command command)
        {
            return await Mediator.Send(command);
        }

        [HttpPut("{id}")]
		public async Task<ActionResult<AppStatusListDto>> Edit(int id, Edit.Command command)
        {
            command.Id = id;
            return await Mediator.Send(command);
        }
